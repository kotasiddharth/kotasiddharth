# Siddharth Kota 

AI Engineer and Roboticist passionate about building intelligent systems, open-source AI agents, and autonomous robotics solutions.

My work focuses on **AI & Machine Learning**, **Engineering**, **Robotics Engineering**, and **Open-Source Development**—contributing to intelligent agent frameworks, control systems, and competitive robotics platforms.

---

## 🚀 Current Focus

**Hermes Agent**  
An extensible AI coding & personal agent with persistent memory and multi-platform support. A terminal-native framework for building intelligent, adaptive systems that learn from every interaction.

- [Hermes Agent Repository](https://github.com/kotasiddharth/hermes-agent) - Core framework and agent runtime
- Full documentation, quickstart, and provider setup guides available

---

## 🤖 AI & Machine Learning

Contributing to open-source AI research and agent development:

- **Large Language Models** — Fine-tuning, evaluation, and deployment optimization
- **Intelligent Agents** — Building autonomous systems with reasoning and learning capabilities
- **Data Pipelines** — Synthetic data generation and dataset curation for model training

---

## 🏆 Robotics & Autonomous Systems

**FRC Robotics (Team 10913)**  
Designing and developing competition-grade control systems and autonomous routines for FIRST Robotics Competition.

- [2026 Robot Code](https://github.com/kotasiddharth/2026-Robot-10913) - Full-season codebase, control algorithms, and autonomous routines
- [Team Github Link](https://github.com/Legacy-Ranch-High-School-Robotics)
- Real-time control systems, vision processing, and competition strategy

---

## 💻 Technical Expertise

**Languages & Frameworks:** Python, Java, Control Theory, Real-time Systems  
**AI/ML Stack:** LLM Fine-tuning, Agent Frameworks, Reinforcement Learning  
**Robotics:** FRC, Motion Control, Computer Vision, Embedded Systems  
**Tools & Infrastructure:** Git, Linux, Docker, CI/CD, Distributed Systems

---

## 📫 Get in Touch

- **Email:** kotasiddharthreddy@gmail.com
- **Twitter/X:** [@sidk0Ta](https://x.com/sidk0Ta)
- **GitHub:** [@kotasiddharth](https://github.com/kotasiddharth)

---

*Open to collaboration on AI systems, robotics projects, and open-source initiatives.*
